...
const DB_INIT = {
    ...
    aiChat: [
        { sender: "ai", text: "Hello! I am your RetailIQ Business Copilot. I analyze your POS registers, Shopify linkages, and stock flows to optimize your operational performance. Ask me anything!" }
    ],
    logs: [
        { timestamp: "2026-05-29T10:00:00Z", task: "Database system initialized successfully.", channel: "System", value: 0 },
        { timestamp: "2026-05-29T11:30:00Z", task: "Admin login session established.", channel: "Security", value: 0 }
    ]
};
...
if (!db) {
    ...
    // Migration check to clean legacy Ahmad/Ahmed accounts from existing local storage
    if (db.settings && (db.settings.managerName === "Ahmed" || db.settings.managerName === "Ahmad")) {
        db.settings.managerName = "";
    }
    if (db.aiChat && db.aiChat.length > 0 && db.aiChat[0].text && db.aiChat[0].text.includes("Hello Ahmed!")) {
        db.aiChat[0].text = "Hello! I am your RetailIQ Business Copilot. I analyze your POS registers, Shopify linkages, and stock flows to optimize your operational performance. Ask me anything!";
    }
    if (db.logs) {
        db.logs.forEach(log => {
            if (log.task === "Ahmed login session established.") {
                log.task = "Admin login session established.";
            }
        });
    }
    localStorage.setItem('retailiq_db', JSON.stringify(db));
...
// Dynamically generate the timeframe date string for the last 7 days ending today
const getDynamicTimeframeStr = (lang) => {
...
// Overwrite hardcoded translation keys
TRANSLATIONS.en.dateTimeframe = getDynamicTimeframeStr('en');
TRANSLATIONS.ar.dateTimeframe = getDynamicTimeframeStr('ar');
...
async function performGoogleLogin() {
    ...
    try {
        ...
        if (!config || !config.apiKey || !config.projectId) {
            throw new Error(isAr ? "الرجاء ضبط إعدادات Firebase Config في صفحة الإعدادات أولاً." : "Please configure Firebase in settings panel first.");
        }
...
function renderPOSCatalog() {
    ...
    filtered.forEach(prod => {
        ...
        card.innerHTML = `
            <div class="pos-product-card-img" style="display: flex; align-items: center; justify-content: center; overflow: hidden; height: 100px;">
                ${prod.imageUrl ? `<img src="${prod.imageUrl}" style="width: 100%; height: 100%; object-fit: cover;">` : prod.emoji}
            </div>
        ...
        row.innerHTML = `
            <td>
                <div class="table-product-cell">
                    <div class="table-product-img" style="display: flex; align-items: center; justify-content: center; overflow: hidden;">
                        ${prod.imageUrl ? `<img src="${prod.imageUrl}" style="width: 100%; height: 100%; object-fit: cover;">` : prod.emoji}
                    </div>
                    <div>
...
function resizeAndCompressImageStatic(file) {
...
async function saveManualProduct(e) {
    ...
    if (imageInput && imageInput.files && imageInput.files[0]) {
        try {
            imageUrl = await resizeAndCompressImageStatic(imageInput.files[0]);
        } catch (err) {
    ...
    if (editId) {
        ...
        const newProduct = { id, name, category, price, cost, stock, emoji, sold: 0, imageUrl };
...
function renderAnalyticsView() {
    ...
    const adminName = db.settings.managerName || "Admin";
    const employeeMock = [
        { name: adminName, sales: 18, total: 15410.00, pct: 54 },
    ...
    if (empBody) {
        ...
        employeeMock.forEach(emp => {
            ...
            if (isAr && emp.name === adminName) nameTrans = (db.settings.managerName ? db.settings.managerName : 'المدير') + ' (أنت)';
  ...
  function sendAIChatMessage() {
    ...
    db.orders.forEach(o => {
        o.items.forEach(item => {
            const prod = db.products.find(p => p.id === item.id);
            if (prod) {
                dynamicProfit += item.qty * (prod.price - prod.cost);
            } else {
                dynamicProfit += item.qty * (item.price * 0.45);
            }
        });
    ...
    const netProfit = baselineProfit + dynamicProfit - 1100.00;
    ...
    if (db.settings.geminiApiKey) {
        ...
        - Combined Store Gross Sales: ${db.settings.currency}${totalRevenue.toFixed(2)}
        - Net profit: ${db.settings.currency}${netProfit.toFixed(2)}
        ...
        .then(data => {
            ...
            try {
                ...
                result = isAr ? "عذراً، واجهت مشكلة في تحليل الرد السحابي. يرجى التحقق من صحة مفتاح Gemini API في الإعدادات." : "Apologies, I encountered a response verification error. Please audit your Gemini API key in the settings panel.";
            }
        ...
        .catch(err => {
            ...
            if (dots) dots.style.opacity = 0;
            db.aiChat.push({ sender: "ai", text: isAr ? "عذراً، حدث خطأ في الاتصال بخوادم Gemini. المرجو التحقق من اتصال الإنترنت." : "Connection failed to Google Gemini servers. Defaulting back to local fallback." });
            saveState();
            renderAIChatHistory();
        });
        ...
        setTimeout(() => {
            ...
            if (isAr) {
                const managerNameText = db.settings.managerName || "أدمن";
                if (normalizedText.includes('خصم') || normalizedText.includes('تنزيل') || normalizedText.includes('تخفيض') || normalizedText.includes('discount')) {
                    const pctMatch = normalizedText.match(/(\d+)/);
                    const percentage = pctMatch ? parseInt(pctMatch[1], 10) : 10;
                    answer = `تم تطبيق خصم بقيمة **${percentage}%** بنجاح! سيتم تحديث أسعار المخزن.`;
                } else if (normalizedText.includes('مرحبا') || normalizedText.includes('مرحباً') || normalizedText.includes('اهلاً') || normalizedText.includes('أهلاً') || normalizedText.includes('اهلا') || normalizedText.includes('صباح الخير') || normalizedText.includes('مساء الخير')) {
                    answer = `أهلاً بك يا ${managerNameText}! 👋 <br>أنا مساعدك الافتراضي الذكي لـ RetailIQ. يمكنك سؤالي عن مبيعاتك، مستويات مخزونك، أرباح اليومية، أو كتابة مسودات لإرسالها للموردين. كيف يمكنني مساعدتك اليوم؟`;
                } else if (normalizedText.includes('باقة') || normalizedText.includes('خطة') || normalizedText.includes('اشتراك') || normalizedText.includes('ترقية') || normalizedText.includes('plan') || normalizedText.includes('subscription')) {
                    answer = `خطة اشتراك متجرك الحالية هي **RetailIQ Starter**. <br><br>• باقة Starter ($19): نقاط بيع واحدة. <br>• باقة Growth ($29): تفعيل مزامنة شوبيفاي المتعددة. <br>• باقة Pro ($49): دعم B2B كامل بدون رسوم معاملات. <br>يمكنك الترقية في أي وقت من شاشة الإعدادات.`;
                } else if (normalizedText.includes('شوبيفاي') || normalizedText.includes('shopify') || normalizedText.includes('اونلاين') || normalizedText.includes('متجر إلكتروني')) {
                    answer = db.shopifyConnected 
                        ? `ربط شوبيفاي **نشط ومتصل حالياً**! 🟢 <br>يتم جلب مبيعات الأونلاين ومزامنة المخازن تلقائياً بانتظام.`
                        : `ربط شوبيفاي **غير نشط حالياً**. 🔴 <br>يرجى التوجه إلى صفحة الإعدادات وتفعيل المزامنة التلقائية باستعمال مفتاح الوصول (API Token) الخاص بك.`;
                } else if (normalizedText.includes('عملة') || normalizedText.includes('ضريبة') || normalizedText.includes('ضرائب') || normalizedText.includes('tax') || normalizedText.includes('currency')) {
                    answer = `العملة المعتمدة في كشوفاتك هي **${cSymbol}** ونسبة الضريبة المحتسبة تلقائياً للمبيعات هي **${db.settings.taxRate}%**.`;
                } else if (normalizedText.includes('كاشير') || normalizedText.includes('pos') || normalizedText.includes('فاتورة') || normalizedText.includes('تسوية')) {
                    const posOrders = db.orders.filter(o => o.channel === "POS");
                    const posSales = posOrders.reduce((sum, o) => sum + o.total, 0);
                    answer = `📊 **ملخص عمليات الكاشير (POS):**<br>- عدد الفواتير المسجلة: **${posOrders.length} فواتير**.<br>- إجمالي مبيعات الكاشير: **${cSymbol}${posSales.toLocaleString(undefined, {minimumFractionDigits: 2})}**.<br>- طرق التسوية المستخدمة تشمل الدفع النقدي الفوري والأقساط المجدولة.`;
                } else if (normalizedText.includes('وصف') || normalizedText.includes('اكتب') || normalizedText.includes('description') || normalizedText.includes('توليد')) {
                    answer = `📝 **وصف تسويقي مقترح للمنتج (Local Fallback):**<br>منتج استثنائي يجمع بين الجودة الفائقة والتصميم العصري الأنيق. مصمم خصيصاً لتلبية احتياجات عملائك اليومية بكفاءة عالية، مما يجعله إضافة مثالية لكتالوج متجرك الإلكتروني. <br><br>*ملاحظة: يمكنك إدخال مفتاح Gemini API في الإعدادات لتفعيل توليد الوصف بالذكاء الاصطناعي السحابي التفاعلي.*`;
                } else if (normalizedText.includes('تحليل') || normalizedText.includes('أرباح') || normalizedText.includes('ربح') || normalizedText.includes('profit') || normalizedText.includes('خسارة')) {
                    answer = `📊 **تحليل الأرباح والخسائر المالي:**<br>- إجمالي المبيعات الموحدة: **${cSymbol}${totalRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}**<br>- صافي الأرباح (بعد الخصومات): **${cSymbol}${netProfit.toLocaleString(undefined, {minimumFractionDigits: 2})}**<br>- نسبة هامش الأرباح: تقريباً **26%**.<br>- المنتج الأكثر ربحاً ومبيعاً: **${bestSelling}**.<br>- التلفيات المسجلة: تم إعدام خاتم الماس بقيمة تكلفة مورد **-${cSymbol}1,100.00** بسبب ضرر الشحن الإقليمي.`;
                } else if (normalizedText.includes('مخزن') || normalizedText.includes('بضاعة') || normalizedText.includes('stock') || normalizedText.includes('نقص') || normalizedText.includes('مورد')) {
                    answer = db.products.filter(p => p.stock <= 9).length > 0 
                        ? `السلع التي أوشكت على النفاد حالياً هي: **${lowStockItems}**. <br><br>✍️ **مسودة إيميل طلب توريد جاهزة للإرسال للمورد:**<br><div class="ai-email-draft">الموضوع: طلب توريد سلع عاجل - متجر ${db.settings.businessName}<br>إلى: supply@retailiq-partners.com<br><br>عزيزي المورد،<br>نرجو تقديم طلبية إعادة شحن وتوريد عاجلة للمنتجات التالية:<br>${lowStockItems.split(", ").map(name => `• **${name}** (+50 وحدة)`).join("<br>")}<br><br>يرجى تأكيد موعد الشحن وإرسال الفاتورة.<br>تحياتي،<br>${db.settings.managerName || "مدير المتجر"}</div>`
                        : `مستويات المخزون ممتازة! لا توجد حالياً أي منتجات تحت حد الأمان التشغيلي (9 وحدات).`;
                } else if (normalizedText.includes('إيراد') || normalizedText.includes('مبيعات') || normalizedText.includes('revenue') || normalizedText.includes('sales')) {
                    answer = `إجمالي إيرادات ومبيعات متجرك الموحدة (POS + شوبيفاي) تبلغ **${cSymbol}${totalRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}**. <br>السلعة الأكثر سحباً وطلباً هي **${bestSelling}**.`;
                } else {
                    answer = `لقد قمت بتحليل استفسارك بخصوص: "${text}". <br>إليك كشف الحساب السريع لمتجرك الحالي:<br><br>• 💰 **إجمالي المبيعات:** **${cSymbol}${totalRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}**<br>• 📈 **صافي الأرباح:** **${cSymbol}${netProfit.toLocaleString(undefined, {minimumFractionDigits: 2})}**<br>• 🏆 **السلعة الأكثر طلباً:** **${bestSelling}**<br>• ⚠️ **سلع منخفضة المخزون:** **${lowStockItems || "لا يوجد"}**<br>• 🏢 **حالة المتجر:** متصل بقاعدة البيانات المركزية بنجاح.<br><br>كيف يمكنني مساعدتك بشكل أكبر؟`;
                }
            } else {
                const managerNameText = db.settings.managerName || "there";
                const managerSig = db.settings.managerName || "Store Administrator";
                if (normalizedText.includes('discount') || normalizedText.includes('promo') || normalizedText.includes('coupon')) {
                    const pctMatch = normalizedText.match(/(\d+)/);
                    const percentage = pctMatch ? parseInt(pctMatch[1], 10) : 10;
                    answer = `Successfully applied a **${percentage}%** discount! Inventory prices will reflect it.`;
                } else if (normalizedText.includes('hello') || normalizedText.includes('hi') || normalizedText.includes('hey') || normalizedText.includes('greetings') || normalizedText.includes('welcome')) {
                    answer = `Hello ${managerNameText}! 👋 <br>I am your AI Retail Copilot. I scan retail parameters across categories to help optimize your business. How can I assist you today?`;
                } else if (normalizedText.includes('plan') || normalizedText.includes('subscription') || normalizedText.includes('upgrade') || normalizedText.includes('pricing')) {
                    answer = `Your store is currently running on the **RetailIQ Starter** plan. <br><br>• Starter ($19/mo): Single physical register. <br>• Growth ($29/mo): Multiple active Shopify channel links. <br>• Pro ($49/mo): B2B support with zero transaction fees. <br>You can modify your plan in Settings.`;
                } else if (normalizedText.includes('shopify') || normalizedText.includes('e-commerce') || normalizedText.includes('storefront') || normalizedText.includes('online')) {
                    answer = db.shopifyConnected
                        ? `Shopify Integration is **currently active and connected**! 🟢 <br>Online orders and product warehouse catalogs sync in the background.`
                        : `Shopify Integration is **offline**. 🔴 <br>Please connect your store URL and input your Admin Access token in the Settings panel to activate sync.`;
                } else if (normalizedText.includes('currency') || normalizedText.includes('tax')) {
                    answer = `Your base operating currency is **${cSymbol}** and the active sales tax rate is **${db.settings.taxRate}%**.`;
                } else if (normalizedText.includes('pos') || normalizedText.includes('cashier') || normalizedText.includes('register') || normalizedText.includes('ticket')) {
                    const posOrders = db.orders.filter(o => o.channel === "POS");
                    const posSales = posOrders.reduce((sum, o) => sum + o.total, 0);
                    answer = `📊 **POS Register Analytics:**<br>- Completed POS tickets: **${posOrders.length} orders**.<br>- Total POS Sales yield: **${cSymbol}${posSales.toLocaleString(undefined, {minimumFractionDigits: 2})}**.<br>- Checkout payment methods include Cash POS settlement and Layaway plan options.`;
                } else if (normalizedText.includes('description') || normalizedText.includes('write') || normalizedText.includes('generate')) {
                    answer = `📝 **Suggested Product Description (Local Fallback):**<br>An exceptional product combining durable construction with a sleek, modern aesthetic. Engineered using high-grade materials to ensure longevity, it is the perfect companion for your customers' everyday lifestyle.<br><br>*Note: Add a Gemini API key in Settings to activate interactive generative AI features.*`;
                } else if (normalizedText.includes('revenue') || normalizedText.includes('sales') || normalizedText.includes('profit') || normalizedText.includes('earn') || normalizedText.includes('loss')) {
                    answer = `Based on active ledgers, your total **Gross Revenue** is **${db.settings.currency}${totalRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}** across **325** completed orders. Your **Net Profit** stands at **${db.settings.currency}${netProfit.toLocaleString(undefined, {minimumFractionDigits: 2})}** (an average profit margin index of **26%**).`;
                } else if (normalizedText.includes('stock') || normalizedText.includes('inventory') || normalizedText.includes('reorder') || normalizedText.includes('alert') || normalizedText.includes('low') || normalizedText.includes('supplier')) {
                    answer = db.products.filter(p => p.stock <= 9).length > 0
                        ? `The following products are currently low in stock: **${lowStockItems}**. <br><br>✍️ **Draft restock PO email to your supplier:**<br><div class="ai-email-draft">Subject: Critical Restock Order Request - ${db.settings.businessName}<br>To: supply@retailiq-partners.com<br><br>Dear Supplier,<br>We would like to place an urgent restock request for the following items:<br>${lowStockItems.split(", ").map(name => `• **${name}** (+50 units)`).join("<br>")}<br><br>Please confirm shipment dates and send the invoice.<br>Best regards,<br>${managerSig}</div>`
                        : `All warehouse stock levels are healthy! No items are currently under the replenishment threshold (9 units).`;
                } else {
                    answer = `I have analyzed your query regarding: "${text}". <br>Here is a quick operational digest of your business:<br><br>• 💰 **Consolidated Revenue:** **${cSymbol}${totalRevenue.toLocaleString(undefined, {minimumFractionDigits: 2})}**<br>• 📈 **Net Profit:** **${cSymbol}${netProfit.toLocaleString(undefined, {minimumFractionDigits: 2})}**<br>• 🏆 **Top Selling SKU:** **${bestSelling}**<br>• ⚠️ **Low Stock warning:** **${lowStockItems || "None"}**<br>• 🏢 **Status:** Successfully connected to the centralized SaaS cloud database.<br><br>How else can I assist you today?`;
                }
            }
            ...
            if (isAr) {
                const managerNameText = db.settings.managerName || "المدير";
                if (normalizedText.includes('إيراد') || normalizedText.includes('ارباح') || normalizedText.includes('إيرادات') || normalizedText.includes('أرباح') || normalizedText.includes('sales') || normalizedText.includes('revenue')) {
                    ...
                    answer = `أهلاً ${managerNameText}! لقد قمت بفحص المخازن. إجمالي السلع في كتالوجك هو **245** سلعة. يوجد حالياً سلع تعاني من انخفاض المخزون (مثل Silk Scarf و Leather Boots). <br><br>بناءً على تفضيلاتك الإدارية، **لقد قمت بصياغة مسودة بريد إلكتروني رسمية جاهزة للنسخ لإرسالها لموردك فوراً لإعادة طلب السلع الناقصة:**
                    <div class="ai-email-draft">
                        ...
                        <strong>${managerNameText}</strong> - مسؤول متجر RetailIQ Store
                    </div>`;
                    ...
                    answer = `مرحباً ${managerNameText}! أنا مساعدك التجاري الافتراضي. يمكنني تحليل المبيعات والمخازن وصياغة إيميلات الموردين. يمكنك سؤالي عن:\n\n• *"كم تبلغ الإيرادات وأرباح متجري؟"* \n• *"ما هو ملخص المخزون وإيميل إعادة الطلب؟"* \n• *"ما هو المنتج الأكثر مبيعاً لدي؟"_*;
                } else {
                ...
                if (normalizedText.includes('revenue') || normalizedText.includes('sales') || normalizedText.includes('profit') || normalizedText.includes('earn')) {
                    ...
                    answer = `Hello ${managerNameText}! I have audited your inventories. You hold **245** total items in stock, with several items approaching critical limits (e.g. Silk Scarf, Leather Boots). <br><br>As requested, **I have dynamically drafted a professional supplier restock email for you to copy and send:**
                    <div class="ai-email-draft">
                        ...
                        Best regards,<br>
                        <strong>${managerSig}</strong> - RetailIQ Store
                    </div>`;
                    ...
                    answer = `Hello ${managerNameText}! I am your AI Retail Copilot. I scan retail parameters across categories to help optimize your business. You can ask me:\n\n• *"What is my total revenue and profit margin?"*\n• *"Which products have low stock status and supplier draft?"*\n• *"What is my best selling product?"*`;
                } else {
...
function generateWeeklyStoreReport() {
    ...
    if (isAr) {
        ...
        container.innerHTML = `
            <div style="font-size: 0.85rem; line-height: 1.5; color: var(--text-dark);">
                <p style="margin-bottom: 12px; font-weight: 500;">
                    مرحباً <strong>${managerNameText}</strong>، إليك التقرير والتحليل الأسبوعي الشامل لأداء متجرك ومخزونك التجاري المحتسب تلقائياً من العمليات النشطة:
                </p>
        ...
        `;
        document.getElementById('email-weekly-report-btn').innerText = `إرسال التقرير الأسبوعي لـ ${managerNameText} 📧`;
    } else {
        const managerNameText = db.settings.managerName || "Administrator";
        container.innerHTML = `
            <div style="font-size: 0.85rem; line-height: 1.5; color: var(--text-dark);">
                <p style="margin-bottom: 12px; font-weight: 500;">
                    Hello <strong>${managerNameText}</strong>, here is your customized Weekly Operating Digest & performance audit compiled dynamically from active retail ledger properties:
                </p>
        ...
        `;
        document.getElementById('email-weekly-report-btn').innerText = `Email Digest to ${managerNameText} 📧`;
    }
...
function emailWeeklyReportToAhmed() {
    const isAr = (db.settings.language === 'ar');
    const emailStr = db.settings.managerName ? `${db.settings.managerName.toLowerCase().replace(/\s+/g, '')}@retailiq.com` : "manager@retailiq.com";
    const msg = isAr ? 
        `تم إرسال التقرير الأسبوعي بنجاح إلى البريد الإلكتروني ${emailStr}!` :
        `Weekly digest successfully compiled and dispatched to ${emailStr}!`;
